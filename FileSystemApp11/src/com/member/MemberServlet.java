...
public class MemberServlet extends HttpServlet
{
	...
	protected void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		}else if(uri.indexOf("login_ok.action") != -1)
		{
			...
			//   세션을 초기화하여 다시 생성하게 된다.
			//   즉, 로그인을 수행할 떄에만 true 값을 부여해야한다.
		...
		}else if(uri.indexOf("logout.action")!= -1) 
		{
			...
			// 로그아웃 이후 다시 요청 할 수 있는 페이지 안ㄴ내
			response.sendRedirect(cp);
			
			
		}		
		else if(uri.indexOf("created.action") != -1) 
		{
		...
		}else if (uri.indexOf("update.action") != -1) 
		{
			...
			if(dto == null) // 세션에서 지워진 상황
			{
				// 넘겨받은 세션 정보로 회원 데이터를 불러 올수 없는 상황
